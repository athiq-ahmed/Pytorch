...
for epoch in range(num_epochs):
    for i, (images, labels) in enumerate(train_dataloader):
        ...
        if (i + 1) % 100 == 0:
            ...
            writer.add_scaler('training_loss', running_loss/100, epoch*n_total_steps*i)
            writer.add_scaler('training_acc', running_correct/100, epoch*n_total_steps*i)
            running_loss = 0.0
            running_correct = 0
...
with torch.no_grad():
    n_correct = 0
    n_samples = 0
    for images, labels1 in test_dataloader:
        images = images.reshape(-1, 28 * 28).to(DEVICE)
